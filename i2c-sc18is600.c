...
#define SC18IS600_CMD_RDBLK	0x01 /* Read N bytes from I2C-bus device */
...
#define SC18IS600_BF(name, value) \
	(((value) & ((1 << SC18IS600_##name##_SIZE) - 1)) \
						<< SC18IS600_##name##_OFFSET)
#define SC18IS600_BFEXT(name, value) \
	(((value) >> SC18IS600_##name##_OFFSET) \
					& ((1 << SC18IS600_##name##_SIZE) - 1))
#define SC18IS600_BFINS(name, value, old) \
	(((old) & ~(((1 << SC18IS600_##name##_SIZE) - 1) \
		<< SC18IS600_##name##_OFFSET)) | SC18IS600_BF(name, value))
...
void debug_spi_msg(struct sc18is600_i2c *i2c, u8 num)
{
	...
	for (i = 1; i < num; i++)
		snprintf(debug_str, 256, "%s, %02x", debug_str,
							i2c->spi_tx_buf[i]);
...
static s32 sc18is600_spi_msg_write(struct sc18is600_i2c *i2c, u8 len)
{
	...
	ret = spi_write(i2c->spi_dev, i2c->spi_tx_buf, len);
	if (ret < 0) {
	...
	case SC18IS600_CMD_WRAWR:
		wait_for_completion_timeout(&i2c->done,
					msecs_to_jiffies(CMD_RESP_TIME));
		ret_reg_rd = sc18is600_read_reg(i2c, SC18IS600_I2CSTAT);
		if (ret_reg_rd < 0)
			dev_err(i2c->dev,
				"Can't read I2CStat internal register!\n");
...
static s32 sc18is600_xfer(struct i2c_adapter *adap, u16 addr,
				 ...
				 const char *buf, size_t count)
{
	...
	ret = sc18is600_write_reg(i2c, (u8)tmp>>8, (u8)tmp&0xFF);
	if (ret < 0)
			return (ssize_t)ret;
...
{
	if (at91_set_A_periph(AT91_PIN_PB0, 1) < 0)
		printk(KERN_INFO "Can't setup periph for MISO\n");

	at91_set_deglitch(AT91_PIN_PB0, 0);

	at91_set_A_periph(AT91_PIN_PB1, 1);	/* SPI1_MOSI */
	at91_set_A_periph(AT91_PIN_PB2, 1);	/* SPI1_SPCK */
	...
	/* Request IRQ for pin */
	if (request_irq(gpio_pin, sc18is600_i2c_irq, IRQF_TRIGGER_NONE,
						"i2c-sc18is600", i2c) < 0)  {
